...
const BUCKET = "raiz"; // Set Up your bucket name here
 ...
 */
export async function listFilesFromMinIO(route, bucket = BUCKET) {
 ...
 */
export async function generatePresignedUrl(key, download = false, bucket = BUCKET) {
...
export async function getFiles(route) {
  return await listFilesFromMinIO(route);
}

export async function getPresignedUrl(key, download = false) {
  try {
    const url = await generatePresignedUrl(key, download);
    return { url };
  } catch (err) {
